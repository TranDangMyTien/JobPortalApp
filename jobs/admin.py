...
from jobs.dao import get_gender_distribution
from jobs.models import (User, Employer, Applicant, Area, EmploymentType, RecruitmentPost, JobApplication, Status,
                         ...
                         Career, Review, PasswordResetToken, Message, Chat, Invoice)
from django import forms
from ckeditor_uploader.widgets import CKEditorUploadingWidget
import cloudinary
from django.urls import path
from jobs import dao, data_analysis
...
class MyAdminSite(admin.AdminSite):
    ...
    def get_urls(self):
        return [
            ...
            path('job_market_stats/', self.job_market_stats),
            path('analysis-results/', self.analysis_results),
        ] + super().get_urls()
    ...
    @method_decorator(user_passes_test(is_admin))
    def job_market_stats(self, request):
        student_jobs_stats = dao.get_student_jobs_stats()
        successful = student_jobs_stats['successful']
        total = student_jobs_stats['total']
        ...
        # total_revenue = total_invoices * 10  # Mỗi hóa đơn 10 USD

        # Thống kê số lượng hóa đơn theo trạng thái thanh toán
        invoices_by_status = Invoice.objects.values('payment_status').annotate(count=Count('id'))

        # Thống kê số lượng hóa đơn hàng tháng
        monthly_invoice_counts = (
            Invoice.objects
            .filter(payment_status='succeeded')  # Chỉ tính hóa đơn đã thanh toán thành công
            .extra(select={'month': "DATE_FORMAT('month', payment_date)"})
            .values('month')
        ...
        monthly_revenue = [
            {
                'month': month['month'],
                'revenue': month['count'] * 10  # Tính doanh thu
            }
            for month in monthly_invoice_counts
        ]
        total_revenue = total_invoices * 10  # Doanh thu từ hóa đơn

        # Thống kê 5 sản phẩm hàng đầu
        top_5_products = (
            Invoice.objects
            .values('product_item')
            .annotate(count=Count('id'))
        ...
        context = {
            ...
            'application_stats': dao.get_application_stats(),
            'expired_tokens': dao.get_expired_tokens_count(),
            'post_reviews_stats': dao.get_post_reviews_stats(),
            'employer_stats': dao.get_employer_stats(),
            'applicant_stats': dao.get_applicant_stats(),
            'recruitment_post_stats': dao.get_recruitment_post_stats(),
            'monthly_application_trend': dao.get_monthly_application_trend(),
            'top_careers': dao.get_top_careers(),
            'gender_distribution': dao.get_gender_distribution(),
            'employer_company_types': dao.get_employer_company_types(),
            'applicant_area_preferences': dao.get_applicant_area_preferences(),
            'avg_applications_per_post': dao.get_average_applications_per_post(),
            'employment_types': dao.get_recruitment_posts_by_employment_type(),
            'salary_range': dao.get_salary_range(),
            'top_employers': dao.get_top_employers(),
            'applicant_experience_distribution': dao.get_applicant_experience_distribution(),
            'application_success_rate': dao.get_application_success_rate(),
            'most_demanded_skills': dao.get_most_demanded_skills(),
            'unsuccessful_student_jobs': unsuccessful,  # Gửi số liệu không thành công vào context
            'labels': labels,
            'counts': counts,
    ...
    @method_decorator(user_passes_test(is_admin))
    def analysis_results(self, request):
        results = data_analysis.main()  # Gọi hàm main để lấy dữ liệu phân tích
        context = {
            'monthly_trends': results['monthly_trends'],
            'success_rate_accuracy': results['success_rate_accuracy'],
            'salary_mse_results': results['salary_mse_results'],
            'application_count_mse': results['application_count_mse'],
            'salary_range_mse_results': results['salary_range_mse_results'],
        }
        return render(request, 'admin/analysis_results.html', context)
...
class InvoiceAdmin(admin.ModelAdmin):
    ...
    list_display = (
    'id', 'user', 'stripe_session_id', 'amount_total', 'currency', 'payment_status', 'payment_date', 'customer_email',
    'product_item')

    # Enable search functionality for specific fields
    search_fields = ('id', 'stripe_session_id', 'user__username', 'customer_email')

    # Add filters for easier navigation in the admin
    list_filter = ('payment_status', 'currency', 'payment_date')

    # Make fields read-only to prevent editing of certain fields directly in the admin
    readonly_fields = ('stripe_session_id', 'payment_date')

    # Customize the display of the user field
    def user_username(self, obj):
        return obj.user.username if obj.user else None

    user_username.short_description = 'Username'  # Optional: Sets the display name in the admin

    # Ordering by payment date descending
